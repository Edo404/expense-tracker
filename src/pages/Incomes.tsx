...
import { PlusCircle, TrendingUp, Search, Calendar, Edit, Trash2 } from 'lucide-react'
...
export default function Incomes() {
  ...
  const handleAddTransaction = (description: string, amount: number, categoryId: string, date: string) => {
    addTransaction('income', amount, categoryId, description, date)
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-600 text-lg">Traccia tutte le tue entrate e guadagni</p>
      </div>      {/* Stats Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-500 text-sm font-medium mb-1">Entrate Totali</p>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-500 text-sm font-medium mb-1">Numero Entrate</p>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-500 text-sm font-medium mb-1">Media Mensile</p>
              <p className="text-3xl font-bold text-teal-600">€{stats.averageMonthly.toFixed(2)}</p>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="flex flex-col md:flex-row gap-4">          <div className="flex-1 relative">
          ...
          <button 
            ...
            className="bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-700 hover:to-teal-700 text-white px-6 py-3 rounded-lg flex items-center gap-2 transition-all shadow-md hover:shadow-lg"
          >
        ...
        <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2">
          ...
          {filteredIncomes.length > 0 ? (
            filteredIncomes.map((income) => (
              <div key={income.id} className="group flex items-center justify-between p-4 hover:bg-gray-50 rounded-lg transition-all border border-gray-100 hover:border-gray-300 hover:shadow-md">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{income.categoryName} • {formatDate(income.date)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <p className="text-xl font-bold text-green-600">+€{income.amount.toFixed(2)}</p>                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
